...
class ClassAutoLoader {
	...
	private function __construct() {
		...
		$l_config = $this->m_tabChemins->children('config');
		$this->m_prefix=  $this->m_tabChemins->config[0]->attributes();
	...
	private function rechercheClasse($p_className) {
		...
		foreach($this->m_tabChemins as $l_key=>$l_chemin) {
			...
			if(is_file($l_chemin .'/'. $this->m_prefix .$p_className .'.php')) {
				require_once $l_chemin .'/'. $this->m_prefix .$p_className .'.php';
			}
		}
