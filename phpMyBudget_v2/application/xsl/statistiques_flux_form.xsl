...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="js.module.sheet">
		...
		<script language="JavaScript" src="application/js/communJson.js" type="text/javascript"/>
	</xsl:template>
	...
	<xsl:template name="Contenu">
		...
		<center>
			...
			<form method="POST" action="#" onsubmit="return soumettreFlux(this);" name="formulaire" id="formulaire">
				<input name="numeroCompte" id="numeroCompte" type="hidden" value="{$NUMEROCOMPTE}"/>
				<table class="formulaire">
					<tr>
						<td>
							<select id="fluxdispos" size="9"/>
						</td>
						<td>
							<select id="fluxselectionnes" size="9" class="obligatoire"/>
						</td>
					</tr>
					<tr>
						<td>
